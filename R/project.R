...
setMethod("initialize", "Project", function(.Object, ...) {
  ...
  if (ellipsis$api) {
    ...
    .Object@config = Config(rawPEP$config)
...
#' @return modified Project object
#' @keywords internal
setGeneric(".modifySamplesFetched", function(object, subsamples)
  standardGeneric(".modifySamplesFetched"))

setMethod(
  ".modifySamplesFetched",
  signature = "Project",
  definition = function(object, subsamples) {
    # object = .removeAttrs(object)
    # object = .appendAttrs(object)
    # object = .duplicateAttrs(object)
    # object = .implyAttrs(object)
    # object = .autoMergeDuplicatedNames(object)
    object = .mergeAttrs(object, subsampleAannotationPaths = NULL, subsamplesTables = subsamples)
    # object = .deriveAttrs(object)
    return(object)
  }
)
...
#' @param subsamplesTable data.table, provided from constructor if PEP is fetched from API
#'
#' @return an object of \code{"\linkS4class{Project}"}
#' @keywords internal
.loadSubsampleAnnotation = function(.Object, path = NULL, subsamplesTable = NULL) {
  if (is.null(subsamplesTable) & !is.null(path)) {
...
#' @return an object of \code{"\linkS4class{Project}"}
#' @keywords internal
.mergeAttrs = function(.Object, subsampleAannotationPaths, subsamplesTables = NULL) {
  if (!is.null(subsamplesTables)) {
    for (sst in subsamplesTables) {
      .Object = .loadSubsampleAnnotation(.Object, subsamplesTable = sst)
    }
    return(.Object)
  }
  ...
  for (p in subsampleAannotationPaths) {
    .Object = .loadSubsampleAnnotation(.Object, path = p)
  }
  return(.Object)
}
